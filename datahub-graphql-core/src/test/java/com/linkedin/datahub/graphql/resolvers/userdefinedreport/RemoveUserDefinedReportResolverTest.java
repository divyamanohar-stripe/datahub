...
public class RemoveUserDefinedReportResolverTest {
    ...
    public void testGetSuccessNoExistingUserDefinedReports() throws Exception {
        ...
        Mockito.when(mockService.exists(Urn.createFromString(TEST_EXISTING_USER_DEFINED_REPORT_URN))).thenReturn(true);

        RemoveUserDefinedReportResolver resolver = new RemoveUserDefinedReportResolver(mockClient, mockService);

        // Execute resolver
        QueryContext mockContext = getMockAllowContext();
        DataFetchingEnvironment mockEnv = Mockito.mock(DataFetchingEnvironment.class);
        Mockito.when(mockEnv.getArgument(Mockito.eq("entityUrn"))).thenReturn(TEST_ENTITY_URN);
        Mockito.when(mockEnv.getArgument(Mockito.eq("userDefinedReportUrn"))).thenReturn(TEST_EXISTING_USER_DEFINED_REPORT_URN);
        Mockito.when(mockEnv.getContext()).thenReturn(mockContext);
        resolver.get(mockEnv).get();

        final UserDefinedReports newUserDefinedReports = new UserDefinedReports().setUserDefinedReports(new UrnArray(Collections.emptyList()));
    ...
    public void testGetEntityClientException() throws Exception {
        ...
        Mockito.doThrow(RemoteInvocationException.class).when(mockClient).ingestProposal(Mockito.any(),
                Mockito.any(Authentication.class));
        RemoveUserDefinedReportResolver resolver = new RemoveUserDefinedReportResolver(mockClient, Mockito.mock(EntityService.class));
