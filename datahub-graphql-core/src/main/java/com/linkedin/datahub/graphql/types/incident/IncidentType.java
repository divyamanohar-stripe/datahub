...
import com.linkedin.datahub.graphql.types.incident.mappers.IncidentMapper;
...
public class IncidentType implements com.linkedin.datahub.graphql.types.EntityType<Incident> {

  static final Set<String> ASPECTS_TO_FETCH = ImmutableSet.of(
      Constants.INCIDENT_KEY_ASPECT_NAME,
      Constants.INCIDENT_PROPERTIES_ASPECT_NAME,
  ...
  @Override
  public List<DataFetcherResult<Incident>> batchLoad(@Nonnull List<String> urns, @Nonnull QueryContext context) throws Exception {
    final List<Urn> incidentUrns = urns.stream()
        .map(this::getUrn)
    ...
    try {
      ...
      return gmsResults.stream()
          .map(gmsResult ->
              gmsResult == null ? null : DataFetcherResult.<Incident>newResult()
