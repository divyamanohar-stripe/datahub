 ...
 */
function getTeamGoodDayMeasure(dataJobEntities: readonly DataJobEntity[]) {
    ...
    dataJobEntities.forEach((dataJobEntity) => {
        ...
        latestRuns.forEach((run) => {
            ...
            } else if (run.missedSLA === false && existing === undefined) {
 ...
 */
function getDataPoints(dataJobOwnerGrouping: Map<any, DataJobEntity[]>, targetTeamsPercentage: number) {
    const allDaysByDate: Map<string, number> = new Map();
    const allTeamsCounts: Map<string, Counts> = new Map();
    dataJobOwnerGrouping.forEach((dataJobEntitiesList) => {
...
const OverallPlot = ({ data, targetRange }: { data: readonly any[]; targetRange: Range }) => {
    if (!data.length) {
        return <></>;
    }

    const color = [blue.primary, purple[3], purple.primary];
...
const TeamsPlot = ({ data, targetRange }: { data: readonly any[]; targetRange: Range }) => {
    if (!data.length) {
        return <></>;
    }
...
type Props = {
    targetRange: Range;
    dataJobOwnerGrouping: Map<any, DataJobEntity[]>;
};

export const HistoricalTimelinessGoodDayMetric = ({ targetRange, dataJobOwnerGrouping }: Props) => {
    ...
    return (
        <div style={{ marginLeft: '15px', marginBottom: '20px' }}>
            <OverallPlot data={data.aggregateData} targetRange={targetRange} />
            <TeamsPlot data={data.dailyData} targetRange={targetTeamsPercentage} />
        </div>
    );
};
